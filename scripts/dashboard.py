...
def api_overview() -> dict:
    ...
    """)
    model_dist = query("""
        SELECT COALESCE(m.model, 'unknown') as model, COUNT(*) messages
        FROM messages m
        JOIN sessions s ON m.session_id = s.session_id
        WHERE s.created_at >= current_date - INTERVAL '30 days'
        GROUP BY m.model ORDER BY messages DESC LIMIT 5
    """)
    sync = query("SELECT MAX(last_run) as last_sync FROM _sync_state")
    return {
        "periods": rows,
        "model_distribution": model_dist,
        "last_sync": sync[0]["last_sync"] if sync else None,
    }


def api_costs() -> list[dict]:
    """Repurposed as session-count-by-project (no token cost data from Cursor)."""
    return query("""
        SELECT project_name, COUNT(*) sessions,
            SUM(message_count) total_msgs,
            ROUND(AVG(message_count), 1) avg_msgs
        FROM sessions WHERE created_at >= current_date - INTERVAL '30 days'
...
def api_sessions() -> list[dict]:
    return query("""
        SELECT strftime(s.created_at, '%Y-%m-%d %H:%M') as started,
            s.project_name, s.message_count msgs,
            mi.model,
            COALESCE(LEFT(s.first_prompt, 120), '') as prompt,
...
def api_trends() -> dict:
    ...
    weekly = query("""
        SELECT DATE_TRUNC('week', s.created_at)::DATE as week,
            COUNT(*) FILTER (WHERE tc.tool_name IN ('Write', 'StrReplace', 'EditNotebook')) as writes,
            COUNT(*) FILTER (WHERE tc.tool_name IN ('Read', 'Glob', 'Grep', 'SemanticSearch')) as reads,
            ROUND(COUNT(*) FILTER (WHERE tc.tool_name IN ('Write', 'StrReplace', 'EditNotebook'))::FLOAT /
                NULLIF(COUNT(*) FILTER (WHERE tc.tool_name IN ('Read', 'Glob', 'Grep', 'SemanticSearch')), 0), 2) as write_read_ratio
        FROM tool_calls tc
        JOIN sessions s ON tc.session_id = s.session_id
...
def api_wrapped() -> dict:
    ...
    """)
    type_map = {
        "Write": "The Architect", "Read": "The Scholar", "Shell": "The Hacker",
        "StrReplace": "The Surgeon", "Grep": "The Detective", "Glob": "The Explorer",
        "Task": "The Orchestrator", "SemanticSearch": "The Researcher",
    }
...
def api_ai_attribution() -> list[dict]:
    """Serve scored_commits data for AI attribution visualization."""
    return query("""
        SELECT commit_hash, branch_name, commit_date,
            lines_added, lines_deleted,
            tab_lines_added, composer_lines_added, human_lines_added,
...
ROUTES = {
    ...
    "/api/ai-attribution": api_ai_attribution,
}
